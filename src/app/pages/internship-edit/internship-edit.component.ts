...
export class InternshipEditComponent implements OnInit{
    ...
    isStartingDateChanged = false;
    isEndingDateChanged = false;
    roleList: Role[] = [];
    ...
    ngOnInit() {
      ...
      this.internshipService.getInternshipById(this.internshipId).subscribe({
        next: (response:any) => {
          ...
          this.internshipForm.patchValue(response.data);
          this.internshipForm.get('endDate')?.setValue(response.data.endingDate);
          this.internshipForm.get('startDate')?.setValue(response.data.startingDate);
          this.internshipForm.get('city')?.setValue(this.setCity(response.data.cityId));
          this.roleList = response.data.internshipRoles;
          this.requirementList = response.data.internshipRequirements;
          this.benefitList = response.data.internshipBenefits;
          this.questionList = response.data.internshipQuestions;
    ...
    validateForm(): boolean {
      ...
      if (this.internshipForm.get('startDate')?.invalid && this.isStartingDateChanged ) {
        return true;
      }
      if (this.internshipForm.get('endDate')?.invalid && this.isEndingDateChanged	) {
        return true;
      }
    ...
    setCity(id: number){
      const city = this.cities.filter((city) => city.cityId === id)[0];
      return city;
    }
